...
import cv2
import warnings
import os  # Import necessário para gerenciar pastas
...
# 1. Criar a pasta 'img' se ela não existir
if not os.path.exists(OUTPUT_FOLDER):
    os.makedirs(OUTPUT_FOLDER)
    print(f"Pasta '{OUTPUT_FOLDER}' criada com sucesso.")

# Carregar imagens (Certifique-se que esses arquivos existem no diretório raiz)
host = cv2.imread('photographer.jpg', 0)
watermark = cv2.imread('lock.jpg', 0)

if host is None or watermark is None:
    print("ERRO: Não foi possível carregar 'photographer.jpg' ou 'lock.jpg'. Verifique os arquivos.")
else:
    ...
    # 5. SALVAR AS IMAGENS NA PASTA 'img'
    print(f"\nSalvando imagens na pasta '{OUTPUT_FOLDER}'...")
    
    path_host = os.path.join(OUTPUT_FOLDER, '1_host_original.jpg')
    path_watermark = os.path.join(OUTPUT_FOLDER, '2_watermark_original.jpg')
    path_result = os.path.join(OUTPUT_FOLDER, '3_watermarked_svd.jpg')
    path_extracted = os.path.join(OUTPUT_FOLDER, '4_extracted_watermark.jpg')

    cv2.imwrite(path_host, host)
    cv2.imwrite(path_watermark, watermark)
    cv2.imwrite(path_result, watermarked_image)
    cv2.imwrite(path_extracted, extracted_watermark)
